...
public class SqliteJournalRepository {
  ...
  public SqliteJournalRepository(
      DataSource dataSource, @Value("${journal.data-dir}") String dataDir) throws IOException {
  ...
  public List<Entry> searchEntries(String query, String category, String from, String to) {
    ...
    if (query != null && !query.isBlank()) {
      sql.append(
          "SELECT e.* FROM entries e JOIN entries_fts f ON e.id = f.entry_id WHERE entries_fts"
              + " MATCH ?");
      params.add(query);
    ...
    return jdbc.sql(sql.toString()).params(params.toArray()).query(this::mapEntry).list();
  }
  ...
  public void saveReminder(Reminder reminder) {
    jdbc.sql(
            "INSERT INTO reminders (id, title, body, remind_at, done, created_at) VALUES (?, ?, ?,"
                + " ?, ?, ?)")
        .params(
  ...
  public void completeReminder(ReminderId id) {
    jdbc.sql("UPDATE reminders SET done = 1 WHERE id = ?").param(id.toString()).update();
  }

  public List<Reminder> listDueReminders() {
    String now = Instant.now().toString();
    return jdbc.sql(
            "SELECT * FROM reminders WHERE remind_at <= ? AND done = 0 ORDER BY remind_at ASC")
        .param(now)
  ...
  public List<Reminder> listUpcomingReminders(int withinHours) {
    ...
    return jdbc.sql(
            "SELECT * FROM reminders WHERE remind_at > ? AND remind_at <= ? AND done = 0 ORDER BY"
                + " remind_at ASC")
        .params(now, until)
        .query(this::mapReminder)
        .list();
