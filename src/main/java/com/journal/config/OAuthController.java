...
public class OAuthController {
    ...
    public OAuthController(@Value("${journal.api-key}") String apiKey, AuthorizationCodeStore codeStore) {
    ...
    public ResponseEntity<String> authorizeForm(
        ...
        String html = """
            ...
            """.formatted(escapeHtml(redirectUri), escapeHtml(state), escapeHtml(codeChallenge), escapeHtml(clientId));
    ...
    public void authorizeSubmit(
        ...
        if (!MessageDigest.isEqual(submittedKey.getBytes(), expectedKey)) {
    ...
    public ResponseEntity<?> token(
        ...
        if (pending == null) {
            return ResponseEntity.badRequest().body(Map.of("error", "invalid_grant", "error_description", "Invalid or expired authorization code"));
        }

        if (!pending.redirectUri().equals(redirectUri)) {
            return ResponseEntity.badRequest().body(Map.of("error", "invalid_grant", "error_description", "redirect_uri mismatch"));
        }

        String computedChallenge = PkceUtil.computeS256Challenge(codeVerifier);
        if (!MessageDigest.isEqual(computedChallenge.getBytes(), pending.codeChallenge().getBytes())) {
            return ResponseEntity.badRequest().body(Map.of("error", "invalid_grant", "error_description", "PKCE verification failed"));
        }
