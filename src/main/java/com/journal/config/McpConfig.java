...
public class McpConfig {
    ...
    @Bean
    public List<McpServerFeatures.SyncPromptSpecification> journalPromptRegistrations(JournalPrompts journalPrompts) {
        var capturePrompt = new McpServerFeatures.SyncPromptSpecification(
            new McpSchema.Prompt(
                "capture",
                "Process a raw voice note or free-text capture into structured journal entries",
                List.of(new McpSchema.PromptArgument("input", "The raw text or transcription to process", true))
            ),
            (exchange, request) -> {
        ...
        var queryPrompt = new McpServerFeatures.SyncPromptSpecification(
            new McpSchema.Prompt(
                "journal_query",
