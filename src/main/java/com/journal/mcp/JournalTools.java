...
public class JournalTools {
  ...
  @Tool(
      description =
          "Store raw capture text (voice note or free-text). Returns the captureId to use in"
              + " subsequent addEntry calls.")
  ...
  @Tool(
      description =
          "Add one structured journal entry. captureId is optional (empty string for direct"
              + " entries). entitiesJson is a JSON array of {name, type} objects. tags is a"
              + " comma-separated string. category must be one of: tech_debt, team_eval,"
              + " feature_refinement, todo, decision, observation, blocker.")
  ...
  @Tool(
      description =
          "Append additional text to an existing entry body, separated by a blank line. Updates"
              + " updated_at.")
  ...
  @Tool(
      description =
          "Create a new reminder. remindAt must be an ISO-8601 datetime string (e.g."
  ...
  @Tool(
      description =
          "Search journal entries. All parameters optional. query uses FTS5 full-text search."
              + " category filters by category. from and to are ISO-8601 date strings for date"
              + " range.")
  ...
  @Tool(
      description =
          "Find all entries mentioning a specific entity by name (person, system, ticket, or"
              + " project).")
  ...
  @Tool(
      description =
          "List the N most recent journal entries. category is optional. limit defaults to 10, max"
              + " 50.")
  ...
  @Tool(
      description =
          "List all due reminders (remind_at in the past, not yet done), sorted by most overdue"
              + " first.")
  ...
  @Tool(
      description =
          "List all pending reminders due within the next N hours, sorted by soonest first.")
  ...
  private List<String> parseTags(String tags) {
    ...
    return List.of(tags.split(",")).stream().map(String::strip).filter(s -> !s.isBlank()).toList();
  }
